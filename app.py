...
CONTENT_DIR = os.environ.get("WWW_CONTENT_DIR", "content")
...
def get_page_list(dir_path, sort_by="date", category=None, limit=5, order="desc"):
    content_path = os.path.join(os.path.split(__file__)[0], CONTENT_DIR, dir_path)
...
def get_page(path: str):
    content_path = os.path.join(os.path.split(__file__)[0], CONTENT_DIR, f"{path}.md")
